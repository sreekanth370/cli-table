...
Table.prototype.toString = function (){
    ...
    function generateRow (items, style) {
        ...
        var ret = '';
        lines.forEach(function (line, index) { // eslint-disable-line no-unused-vars
            if (ret.length > 0) {
    ...
    function string (str, index){
        const convertedStr = String(typeof str == 'object' && str.text ? str.text : str);
        const length = utils.strlen(convertedStr);
        const width = colWidths[index] - (style['padding-left'] || 0) - (style['padding-right'] || 0);
        const align = options.colAligns[index] || 'left';
        // TODO: this is a mess to be refactored
        return repeat(' ', style['padding-left'] || 0) +
            (length == width ? convertedStr : (length < width ?
            	pad(convertedStr, ( width + (convertedStr.length - length) ), ' ', align == 'left' ? 'right' :
            		(align == 'middle' ? 'both' : 'left'))
         		: (truncater ? truncate(convertedStr, width, truncater) : convertedStr))
            )
